    ...
    function ($q, Restangular, UtilService) {
        return {
            ...
            getPublicNews: function (page, size) {
                ...
                Restangular.one('public').one('news').get({'page': page, 'size': size}).then(function (successResult) {
                    deferred.resolve(successResult);
                }, function (failureResult) {
                    deferred.reject(failureResult);
                });
            ...
            notify: function (newsId) {
                var deferred = $q.defer();
                // POST /news/{newsId}/notify
                Restangular.one('news', newsId).one('notify').post().then(function (successResult) {
                    deferred.resolve(successResult);
                }, function (failureResult) {
                    deferred.reject(failureResult);
                });
