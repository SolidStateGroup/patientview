...
angular.module('patientviewApp').controller('SurveysSymptomsCtrl', ['$scope', 'SurveyResponseService', '$filter',
    ...
    function ($scope, SurveyResponseService, $filter, ObservationHeadingService, ObservationService, DocumentService) {
        ...
        var buildChart = function () {
                ...
                for (j = 0; j < questions.length; j++) {
                    ...
                    if (questionAnswerMap[questionType]) {
                        if (questionAnswerMap[questionType].question.type == $scope.questionType) {
                            questionData.push({
                                'date': response.date,
                                'value': questionAnswerMap[questionType].questionOption.score
                            });
                        }
                    }
                }
            ...
            var questionName = _.findWhere($scope.nonCustomQuestions, {
        ...
        var buildTable = function (visibleResponses) {
            ...
            for (i = 0; i < visibleResponses.length; i++) {
                ...
                for (j = 0; j < questions.length; j++) {
                    ...
                    if (tableRows[tableRowIndex] == undefined || tableRows[tableRowIndex] == null) {
                        ...
                        tableRows[tableRowIndex].isCustom = isCustom;
                        tableRows[tableRowIndex].title = questionText;
                    ...
                    if (isCustom && questionOptionText && typeof questionAnswerMap[questionType] !== 'undefined') {
                        tableRows[tableRowIndex].data[0].text = questionAnswerMap[questionType].questionText;
                        tableRows[tableRowIndex].title = questionAnswerMap[questionType].questionText;
                    }
            ...
            $scope.tableRows = _.filter(tableRows, function (row) {
                return row.title;
            });
