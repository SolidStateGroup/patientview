...
public class GroupController extends BaseController<GroupController> {
    ...
    @RequestMapping(value = "/group/{groupId}/joinrequests", method = RequestMethod.GET,
            consumes = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public ResponseEntity<List<JoinRequest>> getJoinRequest(@PathVariable("groupId") Long groupId,
                                                            @RequestParam(value = "statuses", required = false)
                                                            Set<String> statuses)
            throws ResourceNotFoundException {
        return new ResponseEntity(joinRequestService.getByType(groupId, getStatuses(statuses)), HttpStatus.OK);
    }
    ...
    private Set<JoinRequestStatus> getStatuses(Set<String> statuses) {
        ...
        for (String s: statuses) {
            // TODO possible alternation to Jackson settings
            s = s.replaceAll("\"", "");
            s = s.replaceAll("\\]", "");
            s = s.replaceAll("\\[", "");
